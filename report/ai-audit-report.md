...
| AI-18 | 2026-08-21 21:10 | Make CI a strict regression baseline | `vậy có cách nào khác không hay bỏ hết bug ra khỏi test data hay sao ta` then approval to fix the SUT for CI | Added a versioned patch for the pinned SUT submodule, made FR-08 cart setup deterministic, removed CI `continue-on-error`, and reset SUT state between suites. | Human review: ran all three suites locally against the patched SUT. FR-02: 80/80, FR-08: 113/113, FR-14: 91/91 assertions passed. Historical bug reports remain evidence against the original SUT; this is a separate remediated CI baseline. |
| AI-19 | 2026-08-22 00:05 | Produce required CI pass/fail samples | Approval to create an intentional one-assertion failure then restore it | Pushed strict all-pass commit `1e6e571` and temporary failing commit `1998d99`. | Verified GitHub Actions #32505802850 succeeded. Verified #32505900553 failed only because A1-001's expected status was intentionally changed from 200 to 201; the correct assertion is restored in the next commit. |
